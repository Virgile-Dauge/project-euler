...
int main(int argc, char *argv[]) {
  int coucou[20][20] = {{8,  02, 22, 97, 38, 15, 00, 40, 00, 75,
  ...
  for (int i = 0; i < 20; i++) {
    for (int j = 0; j < 16; j++) {
      prod = 1;
      prod =
          coucou[i][j] * coucou[i][j + 1] * coucou[i][j + 2] * coucou[i][j + 3];
      if (prod > maxProd) {
        maxProd = prod;
      }
    }
  ...
  for (int i = 0; i < 20; i++) {
    for (int j = 0; j < 16; j++) {
      prod = 1;
      prod =
          coucou[j][i] * coucou[j][i + 1] * coucou[j][i + 2] * coucou[j][i + 3];
      if (prod > maxProd) {
        maxProd = prod;
      }
    }
  ...
  for (int i = 0; i < 16; i++) {
    for (int j = 0; j < 16; j++) {
      prod = 1;
      prod = coucou[j][i] * coucou[j + 1][i + 1] * coucou[j + 2][i + 2] *
             coucou[j + 3][i + 3];
      if (prod > maxProd) {
        maxProd = prod;
      }
